...
const POLICY: SLAPolicy = {
  ...
  customer_id: null,
  is_default: true,
  // Use distinctive values so a copy-paste error is obvious.
...
describe("addMinutes", () => {
  it("adds the right number of milliseconds", () => {
    const d = new Date("2026-07-19T10:00:00Z");
...
describe("computeSlaTargets", () => {
  it("computes both due times from the policy + severity", () => {
    const created = new Date("2026-07-19T10:00:00Z");
  ...
  it("uses the P3 targets for P3", () => {
    ...
    expect(t.responseDueAt?.toISOString()).toBe("2026-07-19T14:00:00.000Z"); // +4h
    expect(t.resolveDueAt?.toISOString()).toBe("2026-07-20T10:00:00.000Z"); // +24h
  });
});
...
describe("computeSlaBreached", () => {
  ...
  const respDue = new Date("2026-07-19T10:15:00Z"); // 1h45m past
  const resoDue = new Date("2026-07-19T18:00:00Z"); // 6h in the future

  it("is not breached if both deadlines are in the future", () => {
    expect(
      computeSlaBreached({
        status: "in_progress",
        first_response_due_at: new Date("2026-07-19T13:00:00Z").toISOString(),
        resolve_due_at: new Date("2026-07-19T18:00:00Z").toISOString(),
  ...
  it("is NOT breached on response if first_response_at is set", () => {
    expect(
      computeSlaBreached({
        ...
        first_response_due_at: respDue.toISOString(),
        resolve_due_at: resoDue.toISOString(),
        first_response_at: new Date("2026-07-19T10:00:00Z").toISOString(),
        now,
      })
    ).toBe(false);
  });
  ...
  it("is breached on resolution when resolve due passes and ticket is still open", () => {
    expect(
      computeSlaBreached({
        status: "in_progress",
        first_response_due_at: new Date("2026-07-19T13:00:00Z").toISOString(),
        resolve_due_at: new Date("2026-07-19T11:00:00Z").toISOString(), // past
        first_response_at: new Date("2026-07-19T10:00:00Z").toISOString(),
        now,
      })
    ).toBe(true);
  ...
  it("returns on_track when both windows are in the future", () => {
    const s = computeSLAState({
      ticket: {
        severity: "P1",
        status: "new",
        first_response_due_at: new Date("2026-07-19T12:15:00Z").toISOString(),
        resolve_due_at: new Date("2026-07-19T18:00:00Z").toISOString(),
        first_response_at: null,
        sla_breached: false,
      },
  ...
  it("returns response_breached when response window is past and no response", () => {
    const s = computeSLAState({
      ticket: {
        severity: "P1",
        status: "in_progress",
        first_response_due_at: new Date("2026-07-19T10:00:00Z").toISOString(), // 2h past
        resolve_due_at: new Date("2026-07-19T18:00:00Z").toISOString(),
        first_response_at: null,
        sla_breached: true,
      },
  ...
  it("returns resolution_breached when resolve window is past", () => {
    const s = computeSLAState({
      ticket: {
        severity: "P1",
        status: "in_progress",
        first_response_due_at: new Date("2026-07-19T10:00:00Z").toISOString(),
        resolve_due_at: new Date("2026-07-19T11:00:00Z").toISOString(), // 1h past
        first_response_at: new Date("2026-07-19T09:30:00Z").toISOString(),
        sla_breached: true,
      },
      now,
    });
  ...
  it("returns resolution_breached on a closed ticket that breached", () => {
    const s = computeSLAState({
      ticket: {
        ...
        status: "closed",
        first_response_due_at: new Date("2026-07-19T10:00:00Z").toISOString(),
        resolve_due_at: new Date("2026-07-19T11:00:00Z").toISOString(),
        first_response_at: null,
        sla_breached: true,
      },
      now,
