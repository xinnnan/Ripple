...
| INT-024 | Slack actions and signed reply capture require `users.slack_user_id`, but no supported product workflow could set or clear that prerequisite | **Implementation complete; migration 054 pending:** the admin user detail now exposes a dedicated labelled mapping form and bounded API. A service-only command normalizes and uniquely maps one U/W provider ID, serializes with user lifecycle changes, rejects inactive targets, preserves no-ops, and commits exact audit evidence. Legacy mappings are preflighted/canonicalized under a database shape constraint |
...
79. **P0-BZ — closed; deployment confirmed 2026-08-13:** Wire the signed
    ...
    denial, and zero database/Auth residue.
80. **P0-CA — implementation complete; migration 054 pending:** Make the Slack
    ...
    audit evidence, and enforces the provider shape at the database boundary.
    Thirty-one new contracts bring the deterministic suite to 1,146. Three of
