...
| Gate | Result through 2026-08-13 | Meaning |
|---|---|---|
...
| INT-024 | Slack actions and signed reply capture require `users.slack_user_id`, but no supported product workflow could set or clear that prerequisite | **Implementation complete; migration 054 retry pending:** the admin user detail now exposes a dedicated labelled mapping form and bounded API. A service-only command normalizes and uniquely maps one U/W provider ID, serializes with user lifecycle changes, rejects inactive targets, preserves no-ops, and commits exact audit evidence. The first deployment attempt rolled back after finding 19 unusable synthetic `U_HANDLER_*` values. The revised migration explicitly audits and quarantines invalid legacy IDs, rejects ambiguous valid ownership, then enforces the database shape constraint |
...
79. **P0-BZ — closed; deployment confirmed 2026-08-13:** Wire the signed
    ...
    denial, and zero database/Auth residue.
80. **P0-CA — implementation complete; migration 054 retry pending:** Make the Slack
    ...
    selecting enriched actor names from the base audit table.
